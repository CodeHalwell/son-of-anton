...
export class BaseIssueReporterService extends Disposable {
	...
	constructor(
		...
		const fileOnProduct = data.issueSource === IssueSource.SonOfAnton;
	...
	public setEventHandlers(): void {
		...
		this.addEventListener('issue-source', 'change', (e: Event) => {
			...
			if (value === IssueSource.SonOfAnton) {
			...
			} else if (value === IssueSource.SonOfAnton) {
				fileOnProduct = true;
			}
		...
		this.addEventListener('description', 'input', (e: Event) => {
			...
			if (this.issueReporterModel.fileOnExtension() === false) {
				// eslint-disable-next-line no-restricted-syntax
				const title = (<HTMLInputElement>this.getElementById('issue-title')).value;
				this.searchSonOfAntonIssues(title, issueDescription);
			}
		});
	...
	public searchSonOfAntonIssues(title: string, issueDescription?: string): void {
		if (title) {
	...
	public searchIssues(title: string, fileOnExtension: boolean | undefined, fileOnMarketplace: boolean | undefined): void {
		...
		this.searchSonOfAntonIssues(title, description);
	}
	...
	public setSourceOptions(): void {
		...
		sourceSelect.append(this.makeOption(IssueSource.SonOfAnton, localize('vscode', "Son of Anton"), false));
