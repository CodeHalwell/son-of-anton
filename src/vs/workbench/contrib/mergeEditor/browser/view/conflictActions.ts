...
class ActionsContentWidget extends FixedZoneWidget {
	...
	private setState(items: IContentWidgetAction[]) {
		...
		for (const item of items) {
			...
			if (item.action) {
				children.push($('a', { title: item.tooltip, role: 'button', tabindex: 0, onclick: () => item.action!(), onkeydown: (e: KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); item.action!(); } } }, ...title));
			} else {
